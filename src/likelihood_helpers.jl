...
function likelihood_helpers(
    ...
    # model w/ time-varying hosp prob and w/ wastewater - prevlence model
    try
        # Non-constant Rt
        ...
        return (
            ...
            H_prev_means = H_prev_means, log_W_means = log_W_means
        )
...
function likelihood_helpers(
    ...
    try
        ...
        return (
            ...
            H_inc_means = H_inc_means, log_W_means = log_W_means
        )
    catch e
        if warning_bool
            @warn "ODE solver or transformation failed: $e"
...
function likelihood_helpers(
    ...
    try
        ...
        return (
            ...
            H_prev_means = H_prev_means, H_inc_comp_sol = H_inc_comp_sol,
            H_inc_means = H_inc_means, log_W_means = log_W_means
        )
...
function likelihood_helpers(
    ...
    # model w/ non time-varying hosp prob and w/out wastewater - prevalence model
    try
        # Non-constant Rt
        sigma_Rt_non_centered = Rt_params_non_centered[1]
        Rt_init_non_centered = Rt_params_non_centered[2]
        ...
        return (
            success = true,
            E_init = E_init, I_init = I_init, H_init = H_init,
            gamma = gamma, nu = nu, epsilon = epsilon, w = w,
            sigma_hosp = sigma_hosp,
            Rt_t = Rt_t, Rt_init = Rt_init, sigma_Rt = sigma_Rt, alpha_t = alpha_t,
            E_comp_sol = E_comp_sol, I_comp_sol = I_comp_sol, H_comp_sol = H_comp_sol, 
